...
url = 'https://chaco.gov.ar/app/boletin/page/{}'
...
cantidad_paginas = soup.find('ul', attrs={'class':'pagination'}).find_all('li')[-1].find('a')['href'].split('/')[-1]
...
while (i<=cantidad_paginas):
    ...
    filas_boletines = soup.find('div', attrs={'class':'busqueda-resultado'}).find_all('tr')[1:]

    for fila_boletin in filas_boletines:
        tds = fila_boletin.find_all('td')
        date = datetime.strptime(tds[1].text.strip(), '%d.%m.%Y')
        ...
        try:
            url_boletin = 'https://chaco.gov.ar' + tds[-1].find('a')['href']
        except:
            url_boletin = None
