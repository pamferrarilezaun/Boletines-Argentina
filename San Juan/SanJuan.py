...
ARCHIVO_SALIDA_BOLETIN = CARPETA_SALIDA+"boletinSanJuan_{dia}-{mes}-{anio}.pdf"
...
url = 'https://gestionturismo.sanjuan.gob.ar/api/turismo/applicitacionesgobierno.php?opcion=4&mes={}&anio={}'

# Fecha base a partir de la cual existen publicaciones
date = datetime(2016,5,16)

# Obtengo la fecha del ultimo boletin obtenido
for boletin in glob(CARPETA_SALIDA+'*.pdf'):
    date_candidata = datetime.strptime(boletin, 'dataset\\boletinSanJuan_%d-%m-%Y.pdf')
...
while date <= today:
    ...
    r  = s.get(url.format(date.month, date.year), headers = headers)
